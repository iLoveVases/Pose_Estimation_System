...
from definitions import IMAGES_PATHS, IMAGES_PATHS_NPY, IMAGES_PATHS_SCR
...
def gather_data():
    ...
    #  Functions
    def counting(path_to_count):
        path, dirs, files = next(os.walk(path_to_count + "/"))
    ...
    # Main Loop
    for path_to_gather in IMAGES_PATHS:
        number_of_images = counting(path_to_gather)

        for sample in range(1, number_of_images + 1):
            image_path = path_to_gather + f"/{sample}.jpg"
            print(image_path)  # temporary for debug
            ...
            # saving .npy files
            np.save(IMAGES_PATHS_NPY[path_number] + f'/{sample}', keypoints)
            ...
            cv2.putText(img=image_np, text=str(sample), org=(320, 70), fontFace=1, fontScale=5, color=(0, 255, 0),
                        thickness=2)
            cv2.imshow("COLLECTING DATA", image_np)

            # saving screenshots
            os.chdir(IMAGES_PATHS_SCR[path_number])
