...
def count(position):
    path, dirs, files = next(os.walk(f"learning_data/{position}_npy/"))
    file_count = len(files)
    print(f"Got {file_count} {position} npy files to work with")
    return file_count
...
for position_csv in CLASSES:
    ...
    for sample in range(1, data_count + 1):
        ...
        # adding POSE value
        if position_csv == 'sitting':
            X = np.append(arr=X[0], values=0)
        elif position_csv == 'standing':
        ...
        print(file, X)
        # collecting data
        if position_csv == 'sitting' and file == 1:
            data = X
        else:
            data = np.append(data, X, axis = 0)

np.savetxt(csv_file, data, fmt='%1.8f', delimiter=",", header=label, comments=' ')  # fmt='%1.8f' -> 8 precision digits
